...
namespace Palk\MOBS;
use Palk\MOBS\entity\Animal;
use Palk\MOBS\entity\Chicken;
use Palk\MOBS\entity\Cow;
use Palk\MOBS\entity\Pig;
use Palk\MOBS\entity\Sheep;
use Palk\MOBS\entity\BaseEntity;
use Palk\MOBS\entity\Creeper;
use Palk\MOBS\entity\Enderman;
use Palk\MOBS\entity\Monster;
...
class Mobs extends PluginBase implements Listener{
    ...
    public function onCommand(CommandSender $i, Command $cmd, $label, array $sub){
        ...
        switch(array_shift($sub)){
            case "remove":
                if(!$i->hasPermission("MOBS.command.remove")){
                ...
                break;
            case "check":
                if(!$i->hasPermission("MOBS.command.check")){
                    $i->sendMessage(TextFormat::RED . "You do not have permission to use this command");
                    return true;
                }
                $mob = 0;
                ...
                break;
            case "create":
                if(!$i->hasPermission("MOBS.command.create")){
                    $i->sendMessage(TextFormat::RED . "You do not have permission to use this command");
                    return true;
                }
